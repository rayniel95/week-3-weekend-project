...
async function main() {
    ...
    const account = new ethers.Wallet(process.env.toGetVotesPrivateKey!, alchemyProvider);
    ...
    const contractAddress = await contract.getVotes(
        account.address,
    );
    console.log(`you have ${ethers.formatUnits(contractAddress, 18)} votes`);
}
